...
for i = 1:row
    ...
    % 方向ベクトル
    direction = [cog6_direction(i, 2) cog6_direction(i, 3)];
    ...
    friction_vector = cog6_friction * unit_direction;
    neg_friction_vector = -1 * friction_vector;

    % 摩擦力モーメントを計算
    now_friction_force = [neg_friction_vector(1); neg_friction_vector(2); 0];
    position_cog6 = [position_vector(2), position_vector(3), 0];

    friction_moment = cross(position_cog6, now_friction_force);
    friction_moment = friction_moment(3);

    data(end+1) = cog6_direction(i, 1);
