...
csv_list_e1 = dir('data/A/error/typeA_30/*.csv');
csv_list_e2 = dir('data/A/error/typeA_60/*.csv');
...
for i = 1:length(csv_list_e1)
    filepath = fullfile(csv_list_e1(i).folder, csv_list_e1(i).name);

    se2_data = readmatrix(filepath);

    plot3(se2_data(:, 1), se2_data(:,2), se2_data(:,3), "LineWidth", 3, 'Color', 'r');
...
for i = 1:length(csv_list_e2)
    filepath = fullfile(csv_list_e2(i).folder, csv_list_e2(i).name);
