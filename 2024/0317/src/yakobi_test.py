...
class Simulation:
    ...
    def __init__(self):
        ...
        self.j4 = sim.getObject("/robot/j4")
        self.j5 = sim.getObject("/robot/j5")
        self.j6 = sim.getObject("/robot/j6")
        self.coe = sim.getObject("/robot/CoP")

        self.tip = sim.getObject("/target")

        self.robot = sim.getObject("/robot")
    ...
    def simulation(self):
        ...
        while sim.getSimulationTime() < 35:
            ...
            sim.setJointTargetPosition(self.j1, new_theta[0][0])
            sim.setJointTargetPosition(self.j2, new_theta[1][0])
            sim.setJointTargetPosition(self.j3, new_theta[2][0])
            sim.setJointTargetPosition(self.j4, new_theta[3][0])
            sim.setJointTargetPosition(self.j6, z_new_theta)
